...
def main():
    ...
    NodeConn.MAGIC_BYTES = {
        "mainnet": b"\x29\xe4\x9d\xab",
    }

    # create rpc connection to existing node, normally this is done by start_node(...)
    nodes = []
    url = "http://%s:%s@%s:%d" % ('user2952349071', 'pass097049f29dff7cd24c3ee422ca3fb875c3c9c53b764fabf6c2333a8b42ccdbc5b2', '127.0.0.1', int(50498))
    # url = "http://%s:%s@%s:%d" % ('2', '2', '127.0.0.1', int(17772))
    nodes.append(get_rpc_proxy(url, 0, timeout=None))

    test_node = TestNode()
    connections = []
    connections.append(NodeConn('127.0.0.1', 50497, nodes[0], test_node, "mainnet", 170014))
    # connections.append(NodeConn('127.0.0.2', 17770, nodes[0], test_node, "testnet3", 170011))
    test_node.add_connection(connections[0])
    NetworkThread().start()
    test_node.wait_for_verack()
    
    blocks_file = open('blocks-122.hex', 'r')
    ht = 0
